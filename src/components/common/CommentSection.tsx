...
const CommentSection: React.FC<CommentSectionProps> = ({
  ...
  const handleSubmitReply = async (targetId: number) => {
    ...
    try {
      ...
      if (response.data.isSuccess) {
        ...
        setReplyTargetCommentId(null); fetchComments();
      }
    } catch {
      alert("대댓글 등록 오류");
    } finally {
  ...
  const totalActiveCount =
    ...
    activeComments.reduce(
      (sum, c) =>
        sum + c.replies.filter((r) => r.commentStatus === "ACTIVE").length,
      0,
    );
  ...
  return (
    <div className="w-[327px]">
      {/* 댓글 개수 */}
      <div className="mt-[52px] mb-2 text-[16px] text-left">
        댓글 {totalActiveCount}
      </div>
      ...
      <div className="bg-[#F5F5F5] rounded-xl p-4 space-y-4">
        {activeComments.map((comment) => {
          const activeReplies = comment.replies.filter(
            (r) => r.commentStatus === "ACTIVE",
          );
          return (
            <div
              key={comment.commentId}
              className="border-b border-[#E0E0E0] pb-3 space-y-2"
            >
              {/* 원댓글 */}
                ...
                <div className="flex items-start gap-2 flex-1">
                  ...
                  <div className="flex flex-col w-full">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <button
                        onClick={() =>
                          setReplyTargetCommentId(comment.commentId)
                        }

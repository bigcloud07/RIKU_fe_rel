...
function NewRegularRunEdit() {
  ...
  useEffect(() => {
    ...
    const fetchPostData = async () => {
      ...
      if (res.data.isSuccess) {
        ...
        const utcDate = new Date(result.date);         const kstDate = new Date(utcDate.getTime() + 9 * 60 * 60 * 1000); 
        setDateTime({
          date: kstDate,
          time: kstDate.toTimeString().slice(0, 5),         });
        setPacerGroups(
          result.pacers.map((p: any) => ({
            id: p.group,
            pacer: p.pacerName || "",             pacerName: p.pacerName || "",
            distance: p.distance,
            pace: p.pace,
          })),
        );
  ...
  const handleCourseImageUpload = async (
    ...
    const limitedFiles = selectedArray.slice(0, availableSlots); 
    try {
  ...
  const removeCourseImage = (index: number) => {
    const toRemove = coursePreviews[index];
      ...
      setAttachmentUrls((prev) => prev.filter((url) => url !== toRemove));
        ...
        prev.filter((_, i) => i !== index - attachmentUrls.length),
      );
    }
  };
  ...
  const handleSubmit = async () => {
    if (
      ...
      !content ||
      !dateTime.date ||
      pacerGroups.some((g) => !g.pacer || !g.distance || !g.pace)
    ) {
      alert("모든 정보를 입력해주세요.");
      return;
    }
    try {
      ...
      pacerGroups.forEach((group, index) => {
        const matchedPacer = pacers.find(
          (p) => p.name === group.pacer || p.pacerName === group.pacer,
        );
  ...
  const handlePacerChange = (id: string, pacerId: string) => {
    setPacerGroups((prev) =>
      prev.map((group) =>
        group.id === id ? { ...group, pacer: pacerId } : group,
      ),
    );
  };
            ...
            <table className="w-full text-center border-collapse">
              <thead>
                <tr>
                  ...
                  <th className="p-2 text-gray-700 border-b w-[85px]">
                    페이서
                  </th>
                  <th className="p-2 text-gray-700 border-b w-[85px]">거리</th>
                  <th className="p-2 text-gray-700 border-b w-[70px]">
                    페이스
                  </th>
              ...
              <tbody>
                {pacerGroups.map((group) => (
                  <tr key={group.id} className="border-b border-gray-300">
                    <td className="p-2 text-gray-900 text-center w-[50px]">
                      {group.id}
                    </td>
                    <td className="p-2">
                      <select
                        value={group.pacer}                         onChange={(e) =>
                          handlePacerChange(group.id, e.target.value)
                        }
                      ...
                      >
                        ...
                        {pacers.map((pacer) => (
                          <option
                            key={pacer.name || pacer.pacerName}
                            value={pacer.name || pacer.pacerName}
                          >
            ...
            >
              <div className="flex flex-col items-center">
                <div className="text-lg font-semibold">
                  {bottomSheetType === "distance" ? "거리 선택" : "페이스 선택"}
                </div>
          ...
          <div className="relative w-[104px] h-[104px]">
            {mainPreview ? (
              ...
              <label
                ...
                className="w-full h-full border border-dashed border-gray-400 flex items-center justify-center text-gray-500 cursor-pointer rounded-md"
              >
                +
              </label>
            )}

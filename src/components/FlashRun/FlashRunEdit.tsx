  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      if (res.data.isSuccess) {
        ...
        const utcDate = new Date(result.date);         const kstDate = new Date(utcDate.getTime() + 9 * 60 * 60 * 1000);         setAttachmentPreviews(result.attachmentUrls || []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (postImage) formData.append("postImage", postImage);
        ...
        const file = new File([blob], `original_attachment_${i}.jpg`, {
          type: blob.type,
        });
    ...
    <div className="flex flex-col items-center min-h-screen overflow-y-auto">
      ...
      <div className="w-full max-w-md px-4">
        ...
        <DateInput
          ...
          onChange={(date) => setDateTime((prev) => ({ ...prev, date }))}
        />
        ...
        <button
          ...
          className="w-full bg-[#366943] text-white py-3 font-bold rounded-lg mt-4 mb-4"
        >
          수정하기
        </button>
      </div>

...
export type RegularDetail = {
  ...
  userInfo: { userId:number; userName:string; userProfileImg?:string; userRole:string };
  postCreatorInfo: { userName:string; userProfileImg?:string|null };
  groupedParticipants: any[];
  postStatus: "NOW" | "CANCELED" | "CLOSED";
};

export const getRegularPost = async (postId: string): Promise<RegularDetail> => {
  const token = JSON.parse(localStorage.getItem("accessToken") || "null");
  const { data } = await customAxios.get(`/run/regular/post/${postId}`, {
    headers: { Authorization: `${token}` },
  });
  if (!data?.isSuccess) throw new Error(data?.responseMessage || "불러오기 실패");
  return data.result;
};
...
export const getRegularGroups = async (postId: string) => {
  ...
  if (!data?.isSuccess) throw new Error(data?.responseMessage || "그룹 조회 실패");
...
export const patchJoin = async (postId: string, group?: string) => {
  ...
  const { data } = await customAxios.patch(url, {}, { headers: { Authorization: `${token}` }});
  if (!data?.isSuccess) throw new Error(data?.responseMessage || "참여/취소 실패");
  return data;
};
...
export const patchManualAttendance = async (postId: string, payload: {userId:number; isAttend:boolean}[]) => {
  const token = JSON.parse(localStorage.getItem("accessToken") || "null");
...
export const patchFixAttendance = async (postId: string, payload: {userId:number; isAttend:boolean}[]) => {
  const token = JSON.parse(localStorage.getItem("accessToken") || "null");
  const { data } = await customAxios.patch(
